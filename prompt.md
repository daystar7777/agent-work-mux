# Bootstrap: AgentWorkMux Protocol v3 (with Obsidian)
...
`PROJECT_OVERVIEW.md`, `INSTALLATION.md`, `AGENTS.md`, runner notes,
migration notes, or protocol templates unless the user explicitly asks for a
separate human-facing translation.
...
Content in **Section A** below - write verbatim.

### Task 5 - Write `AIMemory/work.log`

Content in **Section B** below - write verbatim.

### Task 6 - Write the v3 metadata and onboarding files
...
- INDEX.md content in **Section F** below
- PROJECT_OVERVIEW.md content in **Section G** below
- INSTALLATION.md content in **Section H** below
- AGENTS.md content in **Section I** below
- goals/ACTIVE.md content in **Section J** below
- goals/INDEX.md content in **Section K** below

These files start mostly empty; they grow as the project ages.

### Task 6.1 - Install a managed reminder block when safe

If your harness has a fixed project instruction file and you can edit it
safely, add or refresh one managed AgentWorkMux reminder block there.
...
template in **Section C**. Include capabilities from Task 1 using the
vendor-neutral vocabulary.

### Task 8 - Obsidian integration (optional, recommended)

Follow **Section E** below. Detect, ask user, install with consent, then instruct
"Open folder as vault: AIMemory/".

### Task 9 - Behavior commitment

From this turn forward, follow the protocol on EVERY user turn. **Section D**
...
- Files you created (absolute paths) - should include INDEX.md, PROTOCOL.md,
  PROJECT_OVERVIEW.md, INSTALLATION.md, AGENTS.md, goals/ACTIVE.md,
  goals/INDEX.md, work.log
...
### If files already exist (re-bootstrap or reinstall)

DO NOT overwrite by default.
If legacy `agent-work-mem` markers are detected, do not perform a fresh
bootstrap and do not duplicate `work.log`. Lazy-load the one-time migration
prompt instead:
<https://raw.githubusercontent.com/daystar7777/agent-work-mux/main/migration.md>

If `AIMemory/INSTALLATION.md` exists and the user is reinstalling, reuse
existing `AIMemory/`, refresh managed reminder blocks, append
`RE_INSTALLED` to `work.log`, and update `reinstall_history`.

If this is just a new session in an already-installed project, read the files,
append `RE_ENGAGED` with your current capabilities, and confirm.

If `INDEX.md` / `PROJECT_OVERVIEW.md` / `INSTALLATION.md` / `AGENTS.md` are
missing (older bootstrap), tell the user to run the upgrade prompt:
<https://raw.githubusercontent.com/daystar7777/agent-work-mux/main/upgrade.md>

---

## Section A - Content for `AIMemory/PROTOCOL.md`
...
- **PROJECT_BOOTSTRAPPED / RE_ENGAGED** - session start markers; include
  the agent's capabilities (see Section A.9 below)
...
## Section B - Content for `AIMemory/work.log`
...
#         UNINSTALLED, RE_INSTALLED, GOAL_CREATED, GOAL_UPDATED,
#         GOAL_COMPLETED, RUNNER_RESULT, CORRECTION
#
# Session-start events (PROJECT_BOOTSTRAPPED, RE_ENGAGED) MUST declare
# Vendor / Harness / Capabilities / Strengths / Context - see PROTOCOL.md Section 9.
# Race-free append discipline - see PROTOCOL.md Section 6.
# Tiered storage (rotation when this file exceeds 1.5x threshold) - see Section 7.
#
...
## Section C - Your first event to append
...
Strengths: <one line>
Context: <token budget or "unknown">
Notes: AgentWorkMux protocol v3 installed. Multi-LLM collaboration enabled.
```
...
Context: 200000
Notes: AgentWorkMux protocol v3 installed.
```
...
To stay race-safe (PROTOCOL.md Section 6), append via a single heredoc per event:
...
If the project uses cloud-synced AIMemory (per Section 6.3), append to your own
...
## Section D - Behavior commitment (memorize this)

From the next user message onward, on EVERY user turn:

1. **Read tier entry-points first, in this order** (per PROTOCOL.md Section 7.7):
...
2. **Check rotation threshold**: count events in work.log; if over 1.5x
   the threshold from INDEX.md (default 75 with threshold 50), rotate
   per Section 7.5 BEFORE doing other work.
...
9. **All new markdown files** go under `AIMemory/` with
   `{slug}.{your-model-id}.md` naming.
10. **Append WORK_END** when done, with status.
11. **For cross-AI handoffs**, write `handoff_*.{your-model-id}.md` per
   AICP, and log a `HANDOFF` event. Update INDEX.md's "Active handoffs"
   section.
12. **When you create or rotate a notable file** (archive, cold digest,
...
## Section E - Obsidian integration (perform during step 7)
...
## Section F - Content for `AIMemory/INDEX.md`
...
## Active goals
- goals/ACTIVE.md -> current goal: (none)
- goals/INDEX.md -> lazy goal history ledger for `/awm goal list`

## Other notable files
- PROTOCOL.md - the rules
- PROJECT_OVERVIEW.md - onboarding primer (read after INDEX.md)
- INSTALLATION.md - install/reinstall lifecycle metadata
...
## Section G - Content for `AIMemory/PROJECT_OVERVIEW.md`
...
## What is this project?
<TODO: 2- sentences. The user can fill this in, or you can ask them
...
## Section H - Content for `AIMemory/INSTALLATION.md`

Write this file on bootstrap. Keep it safe to read and safe to commit if the
user later chooses to track AIMemory. Do not include secrets or private
machine paths.
...
## Schema
- protocol_version: v3
- installer_version: <prompt.md source URL or commit SHA>
- bootstrapped_at: <ISO-8601 timestamp>
- bootstrapped_by: <model-id> @ <harness>
...
|-----------|-------|-------|---------|
| <ISO-8601> | PROJECT_BOOTSTRAPPED | <model-id> | Initial v3 install. |
...
this file. Put private runner or harness overrides under ignored
...
## Section I - Content for `AIMemory/AGENTS.md`

Write this file on bootstrap. It is project-scoped safe routing, not a private
credential store.
...
- `/awm agents add <description>` may append rows to `Project aliases`.
- Example: `/awm agents add 오픈코드의 딥시크` appends
  `opencode-deepseek -> opencode:deepseek:auto`.
- Singular alias form is also supported:
  `/awm agent add 딥시크 오픈코드:딥시크` and
  `/awm agent add 딥시크 오픈코드의 딥시크` append
  `딥시크 -> opencode:deepseek:auto`.
- New aliases are untested until `/awm agents test <alias> --smoke` updates
  `Agent call hints`.
- Keep natural-language source text in Notes when useful. Alias values may be
  local-language nicknames; `Resolves to` values must stay selector-safe.
...
Add or remove aliases only when the user confirms the project's actual agent
setup.

---

## Section J - Content for `AIMemory/goals/ACTIVE.md`

Write this file on bootstrap. It is the small, hot goal cursor. Goal records
carry the durable details; this file only tells the next orchestrator what to
resume.
...
When a goal starts, update this file before dispatch. When a goal completes,
leave it pointing at the completed goal until the next `/awm goal` replaces it,
or set `Current goal-id: none` if the user prefers a clean cursor.

---

## Section K - Content for `AIMemory/goals/INDEX.md`

Write this file on bootstrap. It is the lazy goal history ledger and status
dashboard. Do not read it during ordinary sessions unless the user asks about
previous goals or invokes `/awm goal list` / `/awm status --all`.
...
Update this ledger row whenever a goal is created, updated, paused,
error-paused, or completed. Keep newest updated goals near the top. Token/cost
fields are best-effort: use reported values when available, `estimated:<value>`
when the estimate method is clear, and `unknown` otherwise.
